Feature: Validate empty field updated with NV
  ...
  Scenario Outline: As a user I want to validate empty field updated with NV
    ...
    Then validate empty string field is updated with NV userId "<userId>" ssId "<ssId>" resource_seq "<resource_seq>" entl_seq "<entl_seq>" cert_ind "<cert_ind>" job_desc "<job_desc>"
    Examples:
