...
def when_emptyString(context, userId,ssId,resource_seq,entl_seq,cert_ind,job_desc):
    ...
    for item in List:
        ...
        if item == "empty":
            item.replace("empty", " ")
    return item
@then('validate empty string field is updated with NV userId {userId} ssId {ssId} {resource_seq} {entl_seq} {cert_ind} {job_desc}')
def validatePreProcessing(context,userId,ssId,resource_seq,entl_seq,cert_ind,job_desc):
    ...
    NA=to_str_replace_null(df,when_emptyString(context,userId,ssId,resource_seq,entl_seq,cert_ind,job_desc))
    print(NA)
